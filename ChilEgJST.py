...
def predict_price(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, count=720, interval="minute60")
    df = df.reset_index()
    df['ds'] = df['index']
    df['y'] = df['close']
    data = df[['ds','y']]
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price < target_price * 1.003 and current_price < predicted_close_price * 1.02:
                krw = get_balance("KRW")
                if krw > 5000:
            ...
            jst = get_balance("JST")
            if jst > 100:
                upbit.sell_market_order("KRW-JST", jst*0.9995)
        time.sleep(1)
